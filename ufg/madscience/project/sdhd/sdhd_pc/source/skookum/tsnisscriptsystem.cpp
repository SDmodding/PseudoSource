  ...
  if ( ppResult )
  {
    ...
    if ( vfptr )
    {
      ...
      if ( (m_Flags & 0x4000) != 0 || m_Flags < 0 || (m_Flags & 0x2000) != 0 || (m_Flags & 0x1000) != 0 )
        ComponentOfTypeHK = UFG::SimObjectGame::GetComponentOfTypeHK(vfptr, UFG::NISScriptSystemComponent::_TypeUID);
      else
        ComponentOfTypeHK = UFG::SimObject::GetComponentOfType(vfptr, UFG::NISScriptSystemComponent::_TypeUID);
      v5 = ComponentOfTypeHK;
    }
    m_pSimObject = (char *)v5[1].m_pSimObject;
    v10 = -1i64;
    do
      ++v10;
    while ( m_pSimObject[v10] );
    v11 = AMemory::c_req_byte_size_func(v10 + 1);
    str.i_str_ref_p = AStringRef::pool_new(m_pSimObject, v10, v11, 1u, 0, 1);
...
void __fastcall UFG::TSNISScriptSystem::Mthd_get_active_gameslices(SSInvokedMethod *pScope, SSInstance **ppResult)
{
  ...
  if ( ppResult )
  {
    ...
    v9 = UFG::qString::Tokenize(
           (UFG::qString *)&ComponentOfTypeHK[1].m_BoundComponentHandles.mNode.mNext,
           &list,
           ";",
           0i64);
