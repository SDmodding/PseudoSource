// File Line: 19
// RVA: 0xD98930
void __fastcall hkpMassChangerModifierConstraintAtom::collisionResponseBeginCallback(
        hkpMassChangerModifierConstraintAtom *this,
        hkContactPoint *cp,
        ...
        hkpSimpleConstraintInfoInitInput *inB)
{
  hkVector4f v5; // xmm8
  __int64 v7; // rsi
  hkTransformf *m_transform; // rbx
  hkMatrix3f *p_m_invInertia; // rdi
  hkVector4f v10; // xmm5
  __m128 v11; // xmm4
  hkSimdFloat32 scale; // [rsp+20h] [rbp-A8h] BYREF
  hkMatrix3f v13; // [rsp+30h] [rbp-98h] BYREF
  hkMatrix3f aTb; // [rsp+60h] [rbp-68h] BYREF
  ...
  do
  {
    ...
    if ( (_mm_movemask_ps(
            _mm_and_ps(
              _mm_cmpeq_ps(_mm_shuffle_ps(v5.m_quad, v5.m_quad, 225), v5.m_quad),
              _mm_cmpeq_ps(_mm_shuffle_ps(v5.m_quad, v5.m_quad, 78), v5.m_quad))) & 1) != 0 )
    {
    ...
    {
      m_transform = inA->m_transform;
      p_m_invInertia = &inA->m_invInertia;
      hkMatrix3f::setMulInverseMul(&aTb, &m_transform->m_rotation, &inA->m_invInertia);
      hkMatrix3f::setMul(&v13, &aTb, &m_transform->m_rotation);
      ...
      hkMatrix3f::setMulInverse(&aTb, &v13, &m_transform->m_rotation);
      hkMatrix3f::setMul(p_m_invInertia, &m_transform->m_rotation, &aTb);
    }
    v5.m_quad = (__m128)this->m_factorB;
    inA = inB;
    --v7;
  }
  while ( v7 );
...
// File Line: 60
// RVA: 0xD98AD0
void __fastcall hkpMassChangerModifierConstraintAtom::collisionResponseEndCallback(
        hkpMassChangerModifierConstraintAtom *this,
        hkContactPoint *cp,
...
// File Line: 64
// RVA: 0xD98AE0
void __fastcall hkpCenterOfMassChangerModifierConstraintAtom::collisionResponseBeginCallback(
        hkpCenterOfMassChangerModifierConstraintAtom *this,
        hkContactPoint *cp,
        hkpSimpleConstraintInfoInitInput *inA,
        hkpBodyVelocity *velA,
        hkpSimpleConstraintInfoInitInput *inB,
        hkpBodyVelocity *velB)
{
  ...
  v7 = _mm_add_ps(
         _mm_add_ps(
           ...
           _mm_mul_ps(_mm_shuffle_ps(v6.m_quad, v6.m_quad, 0), inA->m_transform->m_rotation.m_col0.m_quad)),
  ...
  v9 = _mm_add_ps(
         _mm_add_ps(
           ...
           _mm_mul_ps(_mm_shuffle_ps(v8.m_quad, v8.m_quad, 0), inB->m_transform->m_rotation.m_col0.m_quad)),
...
// File Line: 78
// RVA: 0xD98BB0
void __fastcall hkpCenterOfMassChangerModifierConstraintAtom::collisionResponseEndCallback(
        hkpCenterOfMassChangerModifierConstraintAtom *this,
        hkContactPoint *cp,
        float impulseApplied,
        hkpSimpleConstraintInfoInitInput *inA,
        ...
        hkpBodyVelocity *velB)
{
  ...
  v8 = _mm_add_ps(
         _mm_add_ps(
           ...
           _mm_mul_ps(_mm_shuffle_ps(v7.m_quad, v7.m_quad, 0), inA->m_transform->m_rotation.m_col0.m_quad)),
  ...
  v11 = _mm_add_ps(
          _mm_add_ps(
            ...
            _mm_mul_ps(_mm_shuffle_ps(v9.m_quad, v9.m_quad, 0), inB->m_transform->m_rotation.m_col0.m_quad)),
...
// File Line: 91
// RVA: 0xD98C70
void __fastcall hkpSoftContactModifierConstraintAtom::collisionResponseBeginCallback(
        hkpSoftContactModifierConstraintAtom *this,
        hkContactPoint *cp,
        hkpSimpleConstraintInfoInitInput *inA,
        hkpBodyVelocity *velA,
        hkpSimpleConstraintInfoInitInput *inB,
        hkpBodyVelocity *velB)
{
  ...
  ymmword_1424AF280 = *(__m256 *)velB;
}

// File Line: 99
// RVA: 0xD98CB0
void __fastcall hkpSoftContactModifierConstraintAtom::collisionResponseEndCallback(
        hkpSoftContactModifierConstraintAtom *this,
        hkContactPoint *cp,
        float impulseApplied,
        hkpSimpleConstraintInfoInitInput *inA,
        ...
        hkpBodyVelocity *velB)
{
  ...
  velA->m_angular.m_quad = _mm_add_ps(
                             _mm_mul_ps(_mm_sub_ps(v8.m_quad, s_bodyVelocities[0].m_angular.m_quad), v7),
                             s_bodyVelocities[0].m_angular.m_quad);
  v9.m_quad = (__m128)velB->m_angular;
  velB->m_linear.m_quad = _mm_add_ps(
                            _mm_mul_ps(_mm_sub_ps(velB->m_linear.m_quad, *(__m128 *)ymmword_1424AF280.m256_f32), v7),
                            *(__m128 *)ymmword_1424AF280.m256_f32);
  velB->m_angular.m_quad = _mm_add_ps(
                             _mm_mul_ps(_mm_sub_ps(v9.m_quad, *(__m128 *)&ymmword_1424AF280.m256_f32[4]), v7),
                             *(__m128 *)&ymmword_1424AF280.m256_f32[4]);
}

// File Line: 112
// RVA: 0xD98D30
void __fastcall hkpMovingSurfaceModifierConstraintAtom::collisionResponseBeginCallback(
        hkpMovingSurfaceModifierConstraintAtom *this,
        hkContactPoint *cp,
        hkpSimpleConstraintInfoInitInput *inA,
        hkpBodyVelocity *velA,
        hkpSimpleConstraintInfoInitInput *inB,
...
// File Line: 122
// RVA: 0xD98D70
void __fastcall hkpMovingSurfaceModifierConstraintAtom::collisionResponseEndCallback(
        hkpMovingSurfaceModifierConstraintAtom *this,
        hkContactPoint *cp,
        float impulseApplied,
        hkpSimpleConstraintInfoInitInput *inA,
...
// File Line: 133
// RVA: 0xD98DB0
__int64 __fastcall hkpIgnoreModifierConstraintAtom::getConstraintInfo(
        hkpIgnoreModifierConstraintAtom *this,
        hkpConstraintInfo *info)
{
  unsigned int i; // edx
  hkpConstraintData::ConstraintInfo infoOut; // [rsp+20h] [rbp-28h] BYREF
  ...
  if ( this->m_type.m_storage != 29 && this->m_type.m_storage != 1 )
  {
    ...
    hkpConstraintData::getConstraintInfoUtil(this, i, &infoOut);
    info->m_numSolverElemTemps -= infoOut.m_numSolverElemTemps;
    info->m_numSolverResults -= infoOut.m_numSolverResults;
    info->m_sizeOfSchemas -= infoOut.m_sizeOfSchemas;
  }
  return 0i64;
}
...
// File Line: 167
// RVA: 0xD98810
int __fastcall hkpModifierConstraintAtom::addModifierDataToConstraintInfo(
        hkpModifierConstraintAtom *this,
        hkpConstraintInfo *cinfo,
        char *usedFlagsOut)
{
  int result; // eax
...
// File Line: 226
// RVA: 0xD988C0
__int64 __fastcall hkpModifierConstraintAtom::addAllModifierDataToConstraintInfo(
        hkpModifierConstraintAtom *modifier,
        hkpConstraintInfo *cinfo,
        char *usedFlagsOut)
{
  unsigned int v3; // edi
  ...
  {
    v7 = hkpModifierConstraintAtom::addModifierDataToConstraintInfo(i, cinfo, usedFlagsOut);
