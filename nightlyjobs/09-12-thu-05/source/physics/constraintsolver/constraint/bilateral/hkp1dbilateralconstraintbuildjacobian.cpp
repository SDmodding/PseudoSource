// File Line: 34
// RVA: 0xD4ED90
void __fastcall hk1dAngularBilateralConstraintBuildJacobian(
        hkp1dAngularBilateralConstraintInfo *info,
        ...
        hkpConstraintQueryOut *out)
{
  ...
  v9 = _mm_add_ps(
         _mm_add_ps(
           _mm_mul_ps(_mm_shuffle_ps(v7, v7, 85), m_storage->m_scratch1.m_quad),
           _mm_mul_ps(_mm_shuffle_ps(v7, v7, 0), m_storage->m_scratch0.m_quad)),
         _mm_mul_ps(_mm_shuffle_ps(v7, v7, 170), m_storage->m_scratch2.m_quad));
  v10 = _mm_add_ps(
          _mm_mul_ps(_mm_mul_ps(v9, v9), m_storage->m_invMasses.m_quad),
...
// File Line: 54
// RVA: 0xD4F0E0
void __fastcall hk1dLinearFrictionBuildJacobian(
        hkp1dLinearFrictionInfo *info,
        hkpConstraintQueryIn *in,
        hkpConstraintQueryOut *out)
{
  hkVector4f v3; // xmm3
  ...
  __m128 v21; // xmm1
  __m128 v22; // xmm2
  hkpSolverResults *v23; // rax
  float v24; // [rsp+20h] [rbp+8h]

  v3.m_quad = (__m128)info->m_constrainedDofW;
  m_storage = in->m_bodyB.m_storage;
  v5 = in->m_bodyA.m_storage;
  v6 = out->m_jacobianSchemas.m_storage;
  v24 = in->m_virtMassFactor.m_storage;
  v7 = _mm_sub_ps(info->m_pivot.m_quad, m_storage->m_scratch3.m_quad);
  v8 = _mm_sub_ps(info->m_pivot.m_quad, v5->m_scratch3.m_quad);
  *(hkVector4f *)v6 = (hkVector4f)v3.m_quad;
  v9 = _mm_shuffle_ps(v3.m_quad, v3.m_quad, 201);
  v10 = _mm_mul_ps(v9, v7);
  v11 = _mm_sub_ps(_mm_mul_ps(v9, v8), _mm_mul_ps(_mm_shuffle_ps(v8, v8, 201), v3.m_quad));
  v12 = _mm_shuffle_ps(v11, v11, 201);
  v13 = _mm_sub_ps(_mm_mul_ps(_mm_shuffle_ps(v7, v7, 201), v3.m_quad), v10);
  m_quad = v5->m_invMasses.m_quad;
  v15 = _mm_shuffle_ps(v13, v13, 201);
  v16 = _mm_add_ps(
          _mm_add_ps(
            _mm_mul_ps(_mm_shuffle_ps(v15, v15, 85), m_storage->m_scratch1.m_quad),
            _mm_mul_ps(_mm_shuffle_ps(v15, v15, 0), m_storage->m_scratch0.m_quad)),
          _mm_mul_ps(_mm_shuffle_ps(v15, v15, 170), m_storage->m_scratch2.m_quad));
  v17 = _mm_add_ps(
          _mm_add_ps(
  ...
  v18 = _mm_add_ps(
          _mm_mul_ps(_mm_mul_ps(v16, v16), m_storage->m_invMasses.m_quad),
  ...
  v22 = _mm_rcp_ps(v21);
  *(__m128 *)&v6[32] = _mm_shuffle_ps(v16, _mm_unpackhi_ps(v16, v21), 196);
  v23 = info->m_lastSolverResults.m_storage;
  *(__m128 *)&v6[16] = _mm_shuffle_ps(
                         ...
                         196);
  v20.m128_f32[0] = v23->m_internalSolverData * in->m_rhsFactor.m_storage;
  out->m_jacobianSchemas.m_storage = v6 + 64;
  *(_DWORD *)&v6[12] = v20.m128_i32[0];
  v21.m128_f32[0] = *(float *)v6 + 0.00000011920929;
  *(float *)&v6[48] = info->m_maxFrictionForce.m_storage * in->m_microStepDeltaTime.m_storage;
  *(_DWORD *)v6 = v21.m128_i32[0];
  *v6 = (hkpJacobianSchema)9;
}

// File Line: 72
// RVA: 0xD4E6E0
void __fastcall hk1dLinearBilateralConstraintBuildJacobian(
        hkp1dLinearBilateralConstraintInfo *info,
        hkpConstraintQueryIn *in,
        hkpConstraintQueryOut *out)
{
  hkpVelocityAccumulator *m_storage; // rax
  ...
  v8 = _mm_sub_ps(info->m_pivotA.m_quad, m_storage->m_scratch3.m_quad);
  ...
  *(hkVector4f *)v5 = (hkVector4f)v7.m_quad;
  ...
  v20 = _mm_add_ps(
          _mm_add_ps(
            _mm_mul_ps(_mm_shuffle_ps(v19, v19, 85), m_storage->m_scratch1.m_quad),
            _mm_mul_ps(_mm_shuffle_ps(v19, v19, 0), m_storage->m_scratch0.m_quad)),
          _mm_mul_ps(_mm_shuffle_ps(v19, v19, 170), m_storage->m_scratch2.m_quad));
  v21 = _mm_add_ps(
          _mm_add_ps(
  ...
  v22 = _mm_add_ps(
          _mm_mul_ps(_mm_mul_ps(v21, v21), m_quad),
          _mm_mul_ps(_mm_mul_ps(v20, v20), m_storage->m_invMasses.m_quad));
  v23 = _mm_shuffle_ps(
          v22,
          _mm_unpackhi_ps(v22, _mm_max_ps(_mm_add_ps(m_quad, m_storage->m_invMasses.m_quad), (__m128)xmmword_141A712D0)),
          196);
...
// File Line: 98
// RVA: 0xD4F340
void __fastcall hk1dLinearBilateralConstraintBuildJacobianWithCustomRhs_noSchema_inline(
        hkp1dLinearBilateralConstraintInfo *info,
        hkpConstraintQueryIn *in,
        ...
        float customRhs)
{
  ...
  m_quad = info->m_constrainedDofW.m_quad;
  m_storage = in->m_bodyB.m_storage;
  v6 = in->m_bodyA.m_storage;
  v26 = in->m_virtMassFactor.m_storage;
  v7.m_quad = (__m128)info->m_pivotA;
  jac->m_linear0.m_quad = m_quad;
  v8 = _mm_sub_ps(v7.m_quad, m_storage->m_scratch3.m_quad);
  v9 = _mm_sub_ps(v7.m_quad, v6->m_scratch3.m_quad);
  v11 = _mm_shuffle_ps(m_quad, m_quad, 201);
  v12 = _mm_mul_ps(v11, v8);
  v13 = _mm_sub_ps(_mm_mul_ps(v11, v9), _mm_mul_ps(_mm_shuffle_ps(v9, v9, 201), m_quad));
  v14 = _mm_shuffle_ps(v13, v13, 201);
  v15 = _mm_sub_ps(_mm_mul_ps(_mm_shuffle_ps(v8, v8, 201), m_quad), v12);
  ...
  v20 = _mm_add_ps(
          _mm_mul_ps(_mm_mul_ps(v19, v19), m_storage->m_invMasses.m_quad),
          _mm_mul_ps(_mm_mul_ps(v18, v18), v16));
  v21 = _mm_shuffle_ps(
          v20,
  ...
  v25 = customRhs * in->m_rhsFactor.m_storage;
...
// File Line: 110
// RVA: 0xD4E8D0
void __fastcall hk1dLinearBilateralConstraintBuildJacobianWithCustomRhs(
        hkp1dLinearBilateralConstraintInfo *info,
        hkpConstraintQueryIn *in,
        hkpConstraintQueryOut *out,
        float customRhs)
{
  ...
  v9 = _mm_sub_ps(info->m_pivotA.m_quad, m_storage->m_scratch3.m_quad);
  v11 = _mm_shuffle_ps(v5.m_quad, v5.m_quad, 201);
  *(hkVector4f *)v7 = (hkVector4f)v5.m_quad;
  v12 = _mm_mul_ps(v11, v8);
  v13 = _mm_sub_ps(_mm_mul_ps(v11, v9), _mm_mul_ps(_mm_shuffle_ps(v9, v9, 201), v5.m_quad));
  v14 = _mm_shuffle_ps(v13, v13, 201);
  v15 = _mm_sub_ps(_mm_mul_ps(_mm_shuffle_ps(v8, v8, 201), v5.m_quad), v12);
  m_quad = v6->m_invMasses.m_quad;
  v17 = _mm_shuffle_ps(v15, v15, 201);
  v18 = _mm_add_ps(
          _mm_add_ps(
            _mm_mul_ps(_mm_shuffle_ps(v14, v14, 85), m_storage->m_scratch1.m_quad),
            _mm_mul_ps(_mm_shuffle_ps(v14, v14, 0), m_storage->m_scratch0.m_quad)),
          _mm_mul_ps(_mm_shuffle_ps(v14, v14, 170), m_storage->m_scratch2.m_quad));
  v19 = _mm_add_ps(
          _mm_add_ps(
  ...
  v20 = _mm_add_ps(
          _mm_mul_ps(_mm_mul_ps(v19, v19), m_quad),
          _mm_mul_ps(_mm_mul_ps(v18, v18), m_storage->m_invMasses.m_quad));
  v21 = _mm_shuffle_ps(
          v20,
          _mm_unpackhi_ps(v20, _mm_max_ps(_mm_add_ps(m_quad, m_storage->m_invMasses.m_quad), (__m128)xmmword_141A712D0)),
          196);
  v22 = _mm_add_ps(_mm_shuffle_ps(v21, v21, 78), v21);
  v23 = _mm_add_ps(_mm_shuffle_ps(v22, v22, 177), v22);
  v24 = _mm_rcp_ps(v23);
  *(__m128 *)&v7[32] = _mm_shuffle_ps(v19, _mm_unpackhi_ps(v19, v23), 196);
  v25 = customRhs * in->m_rhsFactor.m_storage;
...
// File Line: 122
// RVA: 0xD4EA50
void __fastcall hk1dLinearBilateralConstraintBuildJacobianWithCustomRhs_noSchema(
        hkp1dLinearBilateralConstraintInfo *info,
        hkpConstraintQueryIn *in,
        hkp1Lin2AngJacobian *jac,
        float customRhs)
{
  ...
  v7.m_quad = (__m128)info->m_pivotA;
  jac->m_linear0.m_quad = m_quad;
  v8 = _mm_sub_ps(v7.m_quad, v6->m_scratch3.m_quad);
  v9 = _mm_sub_ps(v7.m_quad, m_storage->m_scratch3.m_quad);
  v11 = _mm_shuffle_ps(m_quad, m_quad, 201);
  v12 = _mm_mul_ps(v11, v8);
  v13 = _mm_sub_ps(_mm_mul_ps(v11, v9), _mm_mul_ps(_mm_shuffle_ps(v9, v9, 201), m_quad));
  v14 = _mm_shuffle_ps(v13, v13, 201);
...
// File Line: 128
// RVA: 0xD4EBB0
void __fastcall hk1dLinearBilateralConstraintUserTauBuildJacobian(
        hkp1dLinearBilateralUserTauConstraintInfo *info,
        hkpConstraintQueryIn *in,
        hkpConstraintQueryOut *out)
{
  hkpVelocityAccumulator *m_storage; // rax
  ...
  v7 = _mm_sub_ps(info->m_pivotA.m_quad, m_storage->m_scratch3.m_quad);
  ...
  *(float *)&m_storage = info->m_tau.m_storage;
...
// File Line: 143
// RVA: 0xD4EEF0
void __fastcall hk1dLinearLimitBuildJacobian(
        hkp1dLinearLimitInfo *info,
        hkpConstraintQueryIn *in,
        hkpConstraintQueryOut *out)
{
  hkpVelocityAccumulator *m_storage; // rax
  ...
  *(hkVector4f *)v5 = (hkVector4f)v6.m_quad;
  v10 = _mm_sub_ps(_mm_mul_ps(v8, v7), _mm_mul_ps(_mm_shuffle_ps(v7, v7, 201), v6.m_quad));
  v11 = _mm_sub_ps(v9.m_quad, m_storage->m_scratch3.m_quad);
  v12 = _mm_shuffle_ps(v10, v10, 201);
  v13 = _mm_sub_ps(_mm_mul_ps(_mm_shuffle_ps(v11, v11, 201), v6.m_quad), _mm_mul_ps(v8, v11));
  m_quad = v4->m_invMasses.m_quad;
  v15 = _mm_shuffle_ps(v13, v13, 201);
  ...
  v17 = _mm_add_ps(
          ...
          _mm_mul_ps(_mm_shuffle_ps(v15, v15, 170), m_storage->m_scratch2.m_quad));
  v18 = _mm_add_ps(
          _mm_mul_ps(_mm_mul_ps(v17, v17), m_storage->m_invMasses.m_quad),
...
// File Line: 181
// RVA: 0xD4F2E0
void __fastcall hkSetCenterOfMassBuildJacobian(
        hkMatrix3f *angToLinVelA,
        hkMatrix3f *angToLinVelB,
        hkpConstraintQueryOut *out)
{
  hkpJacobianSchema *v3; // rax
