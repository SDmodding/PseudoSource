...
// File Line: 101
// RVA: 0xEE4BF8
__int64 __fastcall OSuite::FComputeHashAdd(unsigned int nHash, unsigned __int8 *pData, unsigned __int64 nByteCount)
{
  while ( --nByteCount )
  {
    nHash = (1025 * (nHash + *pData)) ^ ((1025 * (nHash + *pData)) >> 6);
    ++pData;
  }
  return nHash;
}
...
__int64 __fastcall OSuite::FComputeHash<char const *>(const char *const *pszString)
{
  ...
  while ( *v1 )
  {
    v2 = (1025 * ((char)*v1 + v2)) ^ ((1025 * ((char)*v1 + v2)) >> 6);
    ++v1;
  }
